import { AlertTriangle, Archive, CheckCircle2, Edit3, Plus, RotateCcw, Search, Trash2, X } from 'lucide-react';
import { useEffect, useMemo, useRef, useState } from 'react';
import { deleteBusiness, saveBusiness, setBusinessActive } from '../../services/umkmService';
...
const DeleteBusinessDialog = ({ business, onClose, onConfirm, busy }) => {
  const dialogRef = useRef(null);
  ...
  return (
    <div className="admin-dialog-backdrop" role="presentation" onMouseDown={(event) => {
      ...
      <section ref={dialogRef} className="admin-dialog admin-confirm-dialog" role="dialog" aria-modal="true" aria-labelledby="delete-business-title">
        <div className="admin-confirm-icon"><AlertTriangle size={24} aria-hidden="true" /></div>
        <div>
          <p className="admin-eyebrow">HAPUS DATA</p>
          <h2 id="delete-business-title">Hapus UMKM secara permanen?</h2>
          <p>Apakah Anda yakin ingin menghapus <strong>{business.brand || business.name}</strong>? Tindakan ini tidak dapat dibatalkan.</p>
        </div>
        <div className="admin-dialog-actions">
          <button className="admin-secondary-button" type="button" onClick={onClose} disabled={busy}>Batal</button>
          <button className="admin-danger-button" type="button" onClick={onConfirm} disabled={busy}><Trash2 size={18} /> {busy ? 'Menghapus...' : 'Hapus permanen'}</button>
        </div>
      </section>
    </div>
  );
...
const BusinessesPage = ({ businesses, loading, refresh, notify, canManage, initialFilters = {} }) => {
  ...
  const removeBusiness = async (business) => {
    ...
    try {
      ...
      notify('Data UMKM berhasil dihapus.', 'success');
      await refresh();
      setDeleting(null);
    } catch (error) {
      notify(error.message || 'Data UMKM gagal dihapus.', 'error');
    } finally {
      setBusyId(null);
    }
  };
  ...
  return (
    <div className="admin-page-stack">
              ...
              <tbody>
                {visible.map((business) => (
                  <tr key={business.id}>
                    ...
                    {canManage && <td><div className="admin-row-actions"><button className="admin-icon-button" type="button" onClick={() => setEditing(business)} aria-label={`Edit ${business.brand || business.name}`}><Edit3 size={17} /></button><button className="admin-icon-button" type="button" onClick={() => toggleActive(business)} disabled={busyId === business.id} aria-label={business.is_active === false ? 'Aktifkan data' : 'Nonaktifkan data'}>{business.is_active === false ? <RotateCcw size={17} /> : <Archive size={17} />}</button><button className="admin-icon-button admin-icon-button-danger" type="button" onClick={() => setDeleting(business)} disabled={busyId === business.id} aria-label={`Hapus ${business.brand || business.name}`}><Trash2 size={17} /></button></div></td>}
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {deleting && <DeleteBusinessDialog business={deleting} onClose={() => setDeleting(null)} onConfirm={() => removeBusiness(deleting)} busy={busyId === deleting.id} />}
    </div>
  );
};
